import {CircleCloseIcon} from "@/components/shared/Svg";
import {useDeleteStorefrontCategoryItem} from "@/hooks/useStorefront";
import Button from "@/components/shared/Button";
import {StorefrontItem} from "@interfaces/StorefrontInterface";

type Props = {
    closeModal: () => void;
    item: StorefrontItem;
}
const RemoveItem = ({closeModal, item}: Props) => {
    const {isPending, mutate} = useDeleteStorefrontCategoryItem(closeModal);

    const handleRemoveItem = () => {
        mutate({
            id: item._id,
        })
    }

    return (
        <div className={"relative modal-content pt-6"}>
            <button onClick={closeModal} className={"absolute top-4 right-4"}>{CircleCloseIcon}</button>
            <h3 className={"form-modal-title text-center"}>Remove item?</h3>
            <p className={"text-secondary text-center mt-4 "}>Are you sure you want to remove this item from the store? This action is not reversible. </p>

            <div className={"flex gap-6 mt-8"}>
                <button onClick={closeModal} className={"white-btn"}>No, Keep it</button>
                <Button
                    isLoading={isPending}
                    name={"Yes, Remove item"}
                    isValid={true}
                    handleClick={handleRemoveItem}

                />
            </div>
        </div>
    );
};

export default RemoveItem;