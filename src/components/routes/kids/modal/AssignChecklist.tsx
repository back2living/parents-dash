...
const AssignChecklist = ({closeModal}: { closeModal: () => void }) => {
    ...
    return (
        <div className={"relative"}>
            <ModalTop title={"Add checklist"} Icon={CircleCloseIcon} closeModal={closeModal}/>

            <div className={"modal-content flex-column gap-6"}>
                <div>
                    <label className={"auth-label text-primary"} htmlFor="">Checklist Category</label>
                    <div>
                        <Listbox value={selected} onChange={setSelected}>
                            <div className="relative">
                                <Listbox.Button onClick={() => setIsTaskFocused(!isTaskFocused)}
                                    ...
                                    <span
                                        className={`block font-medium text-secondary-dark text-sm`}>{selected ? selected : "Select Category"}</span>
                ...
                <div>
                    <label className={"auth-label text-primary"} htmlFor="">Add Checklist</label>
                    <div>
                        <Listbox value={selected} onChange={setSelected}>
                            <div className="relative">
                                <Listbox.Button onClick={() => setIsCategoriesFocused(!isTaskFocused)} className={`edit-category-btn relative ${isCategoriesFocused ? "border-2 border-[#F07846]" : "border-none"}`}>
                                    <span
                                        className={`block font-medium text-secondary-dark text-sm`}>{selected ? selected : "Select Checklist"}</span>
                                    <span className="pointer-events-none absolute inset-y-0 right-4 flex items-center">
                                        {isCategoriesFocused ? <ChevronUp color={"#868686"}/> : <ChevronDown color={"#868686"}/>}
                                    </span>
                                </Listbox.Button>
                                <Transition
                                    as={Fragment}
                                ...
                                >
                                    <Listbox.Options className="select-dropdown-box max-h-[300px] z-50 left-0">
                                        <div>
                                            {tasks.map((item, personIdx) => (
                                                <Listbox.Option
                                                    onClick={() => setIsCategoriesFocused(false)}
                                                    key={personIdx}
                                                    className={({active}) => `rounded-lg relative cursor-default py-2 text-primary  ${active ? "bg-[#F9FAFB]" : ""}`}
                                                    value={item}
                                                >
