...
const CompletedDoCardPenaltySection = () => {
    ...
    const {data, isPending} = useFetchAllKidDoCards("approved", "penalty", currentKid?._id || "", false);
    ...
    return (
        <div>
            ...
            {!isEmpty && <div className={"grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2 mt-6"}>
                {data?.data?.map((doCard, index) => <PenaltyCard isCompleted doCard={doCard} key={index} />)}
            </div>}
        </div>
    );
};
